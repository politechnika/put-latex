	...
	\end{scope}
	% The offspring
	\draw[black, thin] (1.5, 4) node(offspring){} circle (0.15cm);
	% The offspring after LS
	\filldraw[black] (4, 3) node(offspringAfterLS){} circle (0.15cm);
	% The arrow from the offspring to the offspring after LS
	\begin{scope}[>= angle 60]
	\draw[dashed, thick, ->] (offspring) -- (offspringAfterLS);
	\end{scope}
	% The ideal point
	\draw (8, 0) node [above left=1pt, fill = white] {ideal point};
	\filldraw[fill=black] (8, 0) circle (0.1cm);
	% Legend
	\filldraw[fill=white,draw=black] (0.1, 5.9) -- (3.1, 5.9) -- (3.1, 4.9) -- (0.1, 4.9) -- (0.1, 5.9); % the boundary
